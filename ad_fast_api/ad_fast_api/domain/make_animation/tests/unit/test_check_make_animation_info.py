import pytest
from fastapi import HTTPException
from pathlib import Path
from unittest.mock import patch

from ad_fast_api.domain.make_animation.sources.features import (
    check_make_animation_info as cmai,
)
from ad_fast_api.domain.make_animation.sources.make_animation_schema import ADAnimation
from ad_fast_api.workspace.sources.conf_workspace import (
    get_video_file_name,
    VIDEO_DIR_NAME,
)


def test_valid_animations():
    """
    올바른 애니메이션 값을 인자로 주었을 때 예외가 발생하지 않는지 확인합니다.
    """
    for valid_animation in ADAnimation:
        cmai.check_available_animation(valid_animation)


def test_invalid_animation():
    """
    잘못된 애니메이션 값을 인자로 주었을 때 HTTPException(status_code=400)를 발생하는지 확인합니다.
    """
    invalid_value = "invalid_animation_value"
    with pytest.raises(HTTPException) as excinfo:
        cmai.check_available_animation(invalid_value)
    assert excinfo.value.status_code == 400
    assert "Invalid animation" in excinfo.value.detail


def test_is_video_file_exists_True(tmp_path: Path):
    """
    파일이 존재하는 경우, is_video_file_exists가 (True, relative_path)를 반환하는지 검증합니다.
    """
    base_path = tmp_path
    video_dir_path = base_path / VIDEO_DIR_NAME
    video_dir_path.mkdir(exist_ok=True)
    ad_animation = ADAnimation.dab.value
    video_file_name = get_video_file_name(ad_animation)
    video_file_path = video_dir_path / video_file_name
    video_file_path.touch()  # 파일 생성

    with patch.object(cmai, "get_video_dir_path", return_value=video_dir_path):
        exists, relative_path = cmai.is_video_file_exists(
            base_path=base_path,
            ad_animation=ad_animation,
        )

    expected_relative = video_file_path.relative_to(base_path)
    assert exists is True
    assert relative_path == expected_relative


def test_is_video_file_exists_False(tmp_path: Path):
    """
    파일이 존재하지 않는 경우, is_video_file_exists가 (False, relative_path)를 반환하는지 검증합니다.
    """
    base_path = tmp_path
    video_dir_path = base_path / VIDEO_DIR_NAME
    # 실제 파일은 생성하지 않음
    ad_animation = ADAnimation.dab.value
    video_file_name = get_video_file_name(ad_animation)
    video_file_path = video_dir_path / video_file_name

    with patch.object(cmai, "get_video_dir_path", return_value=video_dir_path):
        exists, relative_path = cmai.is_video_file_exists(
            base_path=base_path,
            ad_animation=ad_animation,
        )

    expected_relative = video_file_path.relative_to(base_path)
    assert exists is False
    assert relative_path == expected_relative
